...
namespace DataTables.Queryable
{
    ...
    /// </summary>
    [Obsolete("use IDataTablesResponse", true)]
    ...
    internal class PagedList<T> : List<T>, IPagedList<T>
    {
        ...
        {
            // pagination is on
            if (queryable.Request.Length > 0)
            {
                int skipCount = queryable.Request.Start;
                int takeCount = queryable.Request.Length;

                TotalCount = queryable.Count();
                //PageNumber = queryable.Request.;
                PageSize = queryable.Request.Length;
