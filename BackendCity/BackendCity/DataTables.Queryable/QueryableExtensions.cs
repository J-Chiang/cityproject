...
namespace DataTables.Queryable
{
    ...
    public static class QueryableExtensions
    {
        ...
        /// </summary>
        public static IDataTablesQueryable<T> ToPagedList<T>(this IEnumerable<T> source, IDataTablesRequest request)
        {
        ...
        public static IDataTablesQueryable<T> ToPagedList<T>(this IQueryable<T> queryable, IDataTablesRequest request)
        {
            return queryable.DataTableFilter(request).ToPagedList();
        }
        ...
        public static IDataTablesQueryable<T> ToPagedList<T>(this IDataTablesQueryable<T> queryable)
        {
            return queryable;
        }
        ...
        /// <param name="request"><see cref="DataTablesRequest{T}"/> instance with filtering parameters.</param>
        /// </summary>
        public static Task<IDataTablesQueryable<T>> ToPagedListAsync<T>(this IEnumerable<T> source, IDataTablesRequest request)
        {
        ...
        /// <param name="request"><see cref="DataTablesRequest{T}"/> instance with filtering parameters.</param>
        /// <returns><see cref="IPagedList{T}"/> intstance.</returns>
        public static Task<IDataTablesQueryable<T>> ToPagedListAsync<T>(this IQueryable<T> queryable, IDataTablesRequest request)
        {
            return Task.Factory.StartNew(() => queryable.DataTableFilter(request).ToPagedList());
        }
        ...
        /// <param name="queryable"><see cref="IDataTablesQueryable{T}"/> instance.</param>
        /// <returns><see cref="IPagedList{T}"/> instance.</returns>
        public static Task<IDataTablesQueryable<T>> ToPagedListAsync<T>(this IDataTablesQueryable<T> queryable)
        {
            return Task.Factory.StartNew<IDataTablesQueryable<T>>(() => QueryableExtensions.ToPagedList(queryable));
        }
        ...
        /// <returns><see cref="IPagedList{T}"/> instance.</returns>
        public static DataTablesResponse Apply<T>(this DataTablesResponse list, Action<T> action)
        {/*
        ...
        public static DataTablesResponse Convert<T, TResult>(this DataTablesResponse source, Func<T, TResult> converter)
        {/*
        ...
        /// <param name="request"><see cref="DataTablesRequest{T}"/> instance that stores filterning dTrequest parameters</param>
        /// <returns><see cref="IDataTablesQueryable{T}"/> with appied <see cref="DataTablesRequest{T}"/></returns>
        public static DataTablesQueryable<T> DataTableFilter<T>(this IQueryable<T> queryable, IDataTablesRequest request)
        {
            IDataTablesQueryable<T> q = queryable.AsDataTablesQueryable(request);
            return q.GlobalSearch() as DataTablesQueryable<T>;
            /*

            // Modify the IQueryable<T> with consecutive steps.
            // If you need to change the order or add extra steps,
            // you should to write own DataTableFilter<T> extension method similarly.
            queryable =
                ...
                // order the IDataTablesQueryable<T> by columns listed in the dTrequest
                //FIXME
                //.Order()
                ;

            return queryable as DataTablesQueryable<T>;
            */
        }
        ...
        /// <param name="request"><see cref="DataTablesRequest{T}"/> instance with dTrequest parameters.</param>
        /// <returns><see cref="IDataTablesQueryable{T}"/> instance.</returns>
        public static IDataTablesQueryable<T> AsDataTablesQueryable<T>(this IQueryable<T> queryable, IDataTablesRequest request)
        {
        ...
        {
            if (!String.IsNullOrEmpty(queryable.Request.Search.Value))
            {
                // searchable columns
                ...
                if (columns.Any())
                {
                    ...
                    {
                        //FIXME , Case Insentive option from Request
                        var x = BuildStringContainsPredicate<T>(c.Field, queryable.Request.Search.Value, true);
                        string expr = c.Search.Value;
                        if (predicate == null)
                        {
                            predicate = PredicateBuilder.Create(x);
                        } else
                        {
                            predicate.Or(x);
                        }
        ...
        public static IDataTablesQueryable<T> ColumnsSearch<T>(this IDataTablesQueryable<T> queryable)
        {
            ...
            var columns = queryable.Request.Columns.Where(c =>
                c.IsSearchable &&
                !String.IsNullOrEmpty(c.Search.Value));
            ...
            if (columns.Any())
            {
                ...
                foreach (var c in columns)
                {
                    var expr = BuildStringContainsPredicate<T>(c.Name, c.Search.Value, true);
        ...
        public static IDataTablesQueryable<T> Order<T>(this IDataTablesQueryable<T> queryable)
        {
            ...
            foreach (var c in columns)
            {
                ...
                if(c.Sort.Direction == SortDirection.Descending)
                {
                    direction = ListSortDirection.Descending;
                }
                queryable = (IDataTablesQueryable<T>)queryable.OrderBy(c.Name, direction, true, alreadyOrdered);
                alreadyOrdered = true;
            }
